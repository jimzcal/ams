...
<div class="cash-status-create">
	...
	<div class="col-lg-3">
		...
		<div class="form-wrapper">
		    ...
		    <?= $form->field($model, 'nca_no')->textInput(['maxlength' => true, 'readonly' => true]) ?>
            ...
            <p><?= empty($model->disbursement_amount) ? number_format(($model3->amount - array_sum(ArrayHelper::getColumn(CashStatus::find(['disbursement_amount'])->where(['nca_no'=>$model2->nca])->all(), 'disbursement_amount')) - $model->disbursement_amount), 2) : number_format($model3->amount - array_sum(ArrayHelper::getColumn(CashStatus::find(['disbursement_amount'])->where(['nca_no'=>$model2->nca])->all(), 'disbursement_amount')), 2) 
            	?></p>

		    <?= $form->field($model, 'disbursement_amount')->textInput(['maxlength' => true, 'value' => $model2->net_amount]) ?>

		    <?= $form->field($model, 'balance')->textInput(['maxlength' => true, 'value' => $find = $model3->amount - $model2->net_amount]) ?>
	...
	<div class="col-lg-9">
		<table class="table table-striped">
            <tr>
                ...
                <td style="font-size: 22px"><?= ($find >= $model2->net_amount) ? '<span style="color: green">WITH SUFFICIENT BALANCE</span>' : '<span style="color: red">WITHOUT SUFFICIENT BALANCE</span>' ?></td>
            </tr>
        </table>
		<table class="table table-bordered">
            <tr>
                ...
                <td>
                	<?= $form->field($model, 'status')->dropDownList([$model2->status => $model2->status, 'Unpaid'=>'Unpaid', 'Paid'=>'Paid', 'Cancelled'=>'Cancelled']) ?>
                </td>
            ...
            <tr>
                <td colspan="4"><label>Particulars</label></br><?= $model2->particulars ?>
                </td>
